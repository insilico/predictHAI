...
bay.d0 <- as.numeric(as.vector(baylor_titers$Matched.Max.day0))
...
# load 1000 filtered genes obtained using Coeffecient of Variation
load("B_ex.cv.fltr.RData")
load("E_ex.cv.fltr.RData")
load("M_ex.cv.fltr.RData")

#### ---- Gussian Mixture Model (GMM) day-0 grouping ---- ####
# Combine Baylor, Stanford, Emory and Mayo data - EM/GMM clustering
# Medium cutoff point should not be greater than any data titers, because we want to create three groups of each data
com.bay.emory.mayo <- c(bay.d0.log2, emory.d0.log2, mayo.d0.log2)
clust_cutpoints <- c(max(com.bay.emory.mayo), max(com.bay.emory.mayo), max(com.bay.emory.mayo))
condition <- "-Inf"
while((any(condition == "-Inf")) | (clust_cutpoints[2] > max(bay.d0.log2)))
{
  bay.emory.mayo.gmm_clust <- normalmixEM(com.bay.emory.mayo, k=3, arbvar = T, epsilon = 1e-08)
  posterior.prob <- data.frame(bay.stan.emory.mayo.gmm_clust$posterior) 
  clust_cutpoints <- sort(c(max(com.bay.emory.mayo[which(colnames(posterior.prob)[apply(posterior.prob,1,which.max)]=="comp.1")])
  ...
  condition <- clust_cutpoints
  clust_cutpoints <- clust_cutpoints
}
clust_cutpoints

plot(bay.emory.mayo.gmm_clust,which=2, axes=FALSE, main2 ="EM/GMM clustering", cex.main = 1, xlab2 ="Combined log2(Day-0)",cex.lab = 1)
...
#### ----- top genes using Coefficient of variation ----- ####
...
# Baylor
exprData <- t(B_ex.cv.fltr)
bay_cv.fltr <- cv.filter(exprData,.02293)$fdata # filtering 200 genes using CoV
dim(bay_cv.fltr)
bay.tg.ids <- rownames(bay_cv.fltr)

### extract top genes from the 1000 filtered genes and calculate their ratio
# Baylor
quotient <- function(x){x[1]/x[2]}
bay.tg.expr <- B_ex.cv.fltr[, bay.tg.ids]
...
# Emory
emory.tg.expr <- E_ex.cv.fltr[, bay.tg.ids]
...
# Mayo
mayo.tg.expr <- M_ex.cv.fltr[, bay.tg.ids]
...
bay.glmnet.fit1 <- cv.glmnet(bay.pair.tg[bay.d0.lr.idx, ], bay.fc.log2.resid.lr, alpha = 0)
...
emory.pred.log2.resid.lr <- predict(bay.glmnet.fit1, s = bay.lambda, newx = emory.pair.tg[emory.d0.lr.idx, ])
...
# Predict on the mayo
mayo.pred.log2.resid.lr <- predict(bay.glmnet.fit1, s = bay.lambda, newx = mayo.pair.tg[mayo.d0.lr.idx, ])
